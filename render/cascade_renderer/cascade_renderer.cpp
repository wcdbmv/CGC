...
void CascadeRenderer::render(QPixmap& pixmap, const QVector<Mesh>& meshes, const QVector<QColor>& colors, const Matrix4x4<double>& view_matrix, int factor) {
	for (int i = 0; i < meshes.size(); ++i) {
		renderMesh(pixmap, meshes[i], view_matrix, colors[i], factor);
	}
}

void CascadeRenderer::renderMesh(QPixmap& pixmap, const Mesh& mesh, const Matrix4x4<double>& view_matrix, const QColor& color, int factor) {
